...
if (diatoms == T) { taxonomy_pa<-bugs.d.m }
if (sba == T) { taxonomy_pa<-bugs.sba.m }
if (hybrid == T) { taxonomy_pa<-bugs.hybrid.m }
...
if (sba == T) { 
  final_metrics=sqldf("select * from metrics left join specialty_metrics using(SampleID)") #Linking metrics and specialty_metrics tables
  saveas<-paste0("sba", ".metrics.csv")
  write.csv(final_metrics, saveas, row.names = F) } #Saving completer metrics dataframe as CSV
if (hybrid == T) { 
  final_metrics=sqldf("select * from metrics left join specialty_metrics using(SampleID)") #Linking metrics and specialty_metrics tables
  saveas<-paste0("hybrid", ".metrics.csv")
  write.csv(final_metrics, saveas, row.names = F) } #Saving completer metrics dataframe as CSV
...
if (diatoms == T) { 
  ...
  final_metrics=sqldf("select * from metrics left join specialty_metrics using(SampleID)") #Linking metrics and specialty_metrics tables
  saveas<-paste0("diatom", ".metrics.csv")
  write.csv(final_metrics, saveas, row.names = F)  #Saving complete metrics dataframe as CSV
