globalVariables(c('STE', 'Class', 'acast', 'oelkup', 'diatom_rf_oe', 'sba_rf_oe', 'hybrid_rf_oe', 'pmmilkup', 
                  'melt', 'value', 'sqldf', 'specnumber', 'ddply', 'summarize', 'NitrogenUptakeMetabolism',
                  'prop.spp.Salinity.BF.d.RF', 'stations', 'diversity', 'OxygenRequirements', 'Salinity', 
                  'Saprobity', 'TrophicState', 'Genus', 'Motility', 'designation', 'Salinity2', 'ZHR', 'CRUS', 
                  'Green', 'rf_out_top', 'BCG', 'BCG12', 'BCG45', 'CRUS', 'IndicatorClass_TP', 'IndicatorClass_Cu', 
                  'IndicatorClass_DOC', 'IndicatorClass_Ref', 'IndicatorClass_TN', 'Heterocystous', 
                  'NitrogenUptakeMetabolism2', 'E', 'O', 'OoverE', 'SampleID', 'met', 'name', 'pnorm', 
                  'results', 'sd', 'taxa', 'val', '.', 'MMI', 'MMI_Percentile', 'OoverE_Percentile', 
                  'Enull', 'Onull', 'OoverE.null', 'Replicate', 'SampleDate', 'StationCode'))

#' @importFrom stats na.omit pnorm predict runif sd var
NULL

#' @importFrom utils capture.output flush.console
NULL

#' @importFrom methods .S4methods new
NULL
